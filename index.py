...
from discord_components import DiscordComponents, ComponentsBot, Select, SelectOption, Button, ButtonStyle, ActionRowt
...
def getinfo(id):
    ...
    he = {
        "Authorization":f"Bot {MTAyMzIxNjYwMDc4ODI0NjU4MA.GOG-ax.bbsNsALYESR4V0wQ6wCReh3DcRfSQyOVDdjaPA}"
    }
...
async def on_ready():
    ...
    print(f"─────────────────────────────────────────────────────")
    print(
        f"메인시스템을 실행 합니다.: {client.user}\n봇 초대 링크 : https://discord.com/oauth2/authorize?client_id={1023216600788246580}&permissions=8&scope=bot")
    print(f"─────────────────────────────────────────────────────")
            ...
            if not int(user_info[3]) >= 3:
                ...
                if not (user_info == None):
                    ...
                    if (res["result"] == True):
                        ...
                        con.close()
                        webhook = DiscordWebhook(
                            url=https://discordapp.com/api/webhooks/1023028768966717510/7lOsbWRzQ1ANdrtCo5nxaBrduW8RRD49yff8Iu8i-vJXKfN14N8GZ405IwHNtFHy9qEN,
...
client.run(MTAyMzIxNjYwMDc4ODI0NjU4MA.GOG-ax.bbsNsALYESR4V0wQ6wCReh3DcRfSQyOVDdjaPA)
